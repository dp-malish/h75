...
$sql='SELECT id,link,link_name,title,meta_d,meta_k,caption,
	...
	if($res['img_s']!=''){$img_s='<a href="/'.$uri_parts[0].'/'.$res['link'].'"><img class="fl five img_video" src="/img/'.$img_dir.'/dbpic.php?id='.$res['img_s'].'" alt="'.$res['img_alt_s'].'" title="'.$res['img_title_s'].' - узнать подробнее..."></a>';}else{$img_s='';}
	$search_content.='<div class="fon_c"><article>'.$img_s.'<a href="/'.$uri_parts[0].'/'.$res['link'].'"><h4>'.$res['caption'].'</h4></a>'.$res['short_text'].'</article><div class="cl"></div></div>';
//***конец
}}mysql_free_result($result);$description.='подробнее...';?>