...
if(!preg_match("/[^0-1]+/",$mob)){($mob)?$file_part='m':$file_part='pc';}else{exit;}}
$im=imagecreatefrompng($img.'.png');
($mob)?imagepng($im,NULL,6):imagepng($im);
imagedestroy($im);
}else{$err=1;}}else{$err=1;}}else{$err=1;}
if($err){header("Content-type: image/png");header('Cache-Control: public, max-age=100');
$im=imagecreatefrompng('../../../img/badimg/badimg.png');imagepng($im,NULL,3);}