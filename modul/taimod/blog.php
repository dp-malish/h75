...
if(!preg_match("/^[0-9а-яёa-z\-]+$/u",$uri_parts[0])){$module='404';}else{$DB=new SQLi();
	$res=$DB->strSQL('SELECT link,title,meta_d,meta_k,caption,img,img_alt,img_title,full_text FROM content WHERE link='.$DB->realEscapeStr($uri_parts[0]));
	...
	$main_content.='<div class="fon_c"><article><h3>'.$res['caption'].'</h3>'.$img.$res['full_text'].'</article><div class="cl"></div></div>';
	}else{$module='404';}
