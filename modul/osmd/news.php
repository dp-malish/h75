<?php
$msg=1;
$title='Новости ОСМД Фортуна - 119';
$description='Новости ОСМД Фортуна - 119. ';
$keywords='новости ОСМДб Фортуна - 119,';
try{if($count_uri_parts>2){throw new Exception();}else{
if(!isset($uri_parts[1])){
	$DB=new SQLi();
	Str_navigation::navigation($uri_parts[0],'news',1,$msg,true);
	$main_content.='<article><h2>Новости ОСМД Фортуна - 119</h2>'.Str_navigation::$navigation;
	...
	if(!Validator::paternStrLink($uri_parts[1])){$bad_link=1;}else{
		...
		if(preg_match("/^[0-9]+$/u",$uri_parts[1])){
			...
			if($res){
				...
				$keywords=$res['meta_k'];
				if($res['img']!=''){
					$img='<img class="fl five br" src="/img/news/dbpic.php?id='.$res['img'].'" alt="'.$res['img_alt'].'" title="'.$res['img_title'].'">';
				}else{$img='';}
